...
-- Risk Flags Table (enhanced version of existing fraud system)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'risk_flags') THEN
...
CREATE OR REPLACE VIEW public.v_payments_daily AS
SELECT 
...
SELECT 
  ...
  SUM(bonus_amount) FILTER (WHERE status = 'completed') as completed_bonus_amount
...
SELECT 
  ...
  SUM(ubt.bonus_amount) as total_cost,
  AVG(ubt.bonus_amount) as avg_cost_per_grant,

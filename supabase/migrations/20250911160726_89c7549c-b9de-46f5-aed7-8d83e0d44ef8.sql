...
CREATE POLICY IF NOT EXISTS "user_devices_self_read" 
  ON public.user_devices FOR SELECT 
  USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "user_devices_admin_read" 
  ON public.user_devices FOR SELECT 
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "device_events_self_read"
  ON public.device_events FOR SELECT
  USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "device_events_admin_read"
  ON public.device_events FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
...
-- System policies for inserting device data
CREATE POLICY IF NOT EXISTS "user_devices_system_insert"
  ON public.user_devices FOR INSERT
  WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "user_devices_system_update"
  ON public.user_devices FOR UPDATE
  USING (true);

CREATE POLICY IF NOT EXISTS "device_events_system_insert"
